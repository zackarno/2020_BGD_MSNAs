...
svychisq_posthoc_test <-
  function(design=hhsvy, independent_var=ind_temp, dependent_var=dep_temp,
           method = "bonferroni",
           round = 4,
           alpha = 0.05,
           ...) {
    ...
    stdres <- as.data.frame.matrix(stdres) %>% filter(!is.na(.))
    adjusted_p_values <- as.data.frame.matrix(adjusted_p_values)
    adjusted_p_values <- adjusted_p_values %>% filter(!is.na(.))
    ...
    colnames <- paste0(dependent_var,"*",dimnames(stdres)[[2]])
    colnames <- append(colnames, c("dep_var_option", "value"), after = 0)
    ...
    results %>% 
      ...
      results[odd_rows, c(3:ncol(results))] <- stdres
      results[even_rows, c(3:ncol(results))] <- NA
      results[odd_rows, 2] <- "residuals"
      results[even_rows, 2] <- "p_value"
      colnames <- paste0(dependent_var,"*",dimnames(stdres)[[2]])
      colnames <- append(colnames, c("dep_var_option", "value"), after = 0)
...
#     colnames <- append(colnames, c("indep_var_option", "value"), after = 0)
#     colnames(results) <- colnames
#     rownames <- dimnames(stdres)[[1]]
#     results[odd_rows, 1] <- rownames
#     results[even_rows, 1] <- rownames
...
# ind_temp<-"I.HH_CHAR.arrival_date.HH"
# dep_temp<- "I.FSL.food_consumption_score.HH"
# svychisq_posthoc_test <-
#   function(design=hhsvy, independent_var=ind_temp, dependent_var=dep_temp,
#            method = "bonferroni",
#            round = 4,
